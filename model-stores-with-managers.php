...
function selectManagesWhatStore($sid) {
    try {
        $conn = get_db_connection();
        $stmt = $conn->prepare("SELECT m.manager_id, manager_name, manager_time, floor, hours, l.level_id, l.store_id  FROM `manager` m join level l on l.manager_id = m.manager_id WHERE l.store_id=?");
